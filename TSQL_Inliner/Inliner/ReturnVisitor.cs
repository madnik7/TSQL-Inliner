    ...
    public class ReturnVisitor : TSqlFragmentVisitor
    {
        ...
        public override void Visit(PrintStatement node)
        {
            if (node.Expression is FunctionCall functionCall)
                node.Expression = ReturnVisitorHandler(functionCall/*, out List<DeclareVariableStatement> outDeclareVariableStatement*/);
            base.Visit(node);
        }
        ...
        private ScalarExpression ReturnVisitorHandler(FunctionCall functionCall/*, out List<DeclareVariableStatement> declareVariableStatement*/)
        {
                ...
                TSqlScript.Batches.FirstOrDefault().Statements.FirstOrDefault() is CreateFunctionStatement createFunctionStatement)
            {
                ...
                {
                    RenameVariablesVisitor renameVariablesVisitor = new RenameVariablesVisitor();
                    for (int i = 0; i < createFunctionStatement.Parameters.Count; i++)
                        renameVariablesVisitor.ReturnVisitorDictionary.Add(createFunctionStatement.Parameters[i], functionCall.Parameters[i]);
